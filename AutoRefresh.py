...
def get_latest_sso_cache_file()
    ...
    if not json_files
        raise FileNotFoundError(fNo JSON cache files found in {cache_dir}. Please run 'aws sso login' first.)
...
def write_credentials_to_file(profile_name, access_key_id, secret_access_key, session_token)
    ...
    profiles[profile_name] = [
        faws_access_key_id = {access_key_id},
        faws_secret_access_key = {secret_access_key},
        faws_session_token = {session_token}
    ]
        ...
        for profile, credentials in profiles.items()
            if profile == sandbox
                for sandbox_profile in [sandbox, sandbox2, sandbox3]
                    file.write(f[{sandbox_profile}]n)
                    for credential in credentials
                        file.write(f{credential}n)
                    file.write(n)
            else
                file.write(f[{profile}]n)
                for credential in credentials
                    file.write(f{credential}n)
                file.write(n)
        
    print(fCredentials for profile '{profile_name}' have been written to {credentials_file}.)
...
def main()
    ...
    for profile_name in profiles
        ...
        try
            ...
            print(fFound cache file {cache_file})
